...
//#include "rk4_II.hpp"
#include "prova_rk_con_argomenti.hpp"
...
int main(){
    ...
    Hooke spring(1); // solo la costante elastica, la lunghezza a riposo è nella classe chain
    ...
    while (window.isOpen())
    {
        ...
        for (int j = 0; j < evolution_of_chains.size(); j++){
            for (int i = 0; i < evolution_of_chains[j].size(); i++){
